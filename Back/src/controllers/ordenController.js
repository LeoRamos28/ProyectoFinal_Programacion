...
export const getOrdenesTecnico = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Fallo interno al listar órdenes del técnico." });
  }
};
...
export const getTecnicosCarga = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Fallo interno al obtener la carga de trabajo." });
  }
};
...
export const createOrden = async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ error: "Faltan datos de la orden o el usuario logueado." });
    }
    ...
    if (id_tecnico_asignado) {
      ...
      if (ordenesAsignadas >= 10) {
        return res
          ...
          .json({
            error:
              "El técnico ya tiene la carga máxima de órdenes pendientes/asignadas.",
          });
      }
    }
  ...
  try {
    ...
    if (!estadoNormalizado || !ESTADOS_PERMITIDOS.includes(estadoNormalizado)) {
      return res.status(400).json({
        error:
          "Estado no válido o no proporcionado. Debe ser: en_proceso, finalizada o cancelada.",
      });
    }
    ...
    if (estadoNormalizado === "finalizada") {
      ...
      if (!solucion_reclamo || solucion_reclamo.length < 5) {
        return res.status(400).json({
          error:
            "La solución del reclamo es obligatoria para finalizar la orden.",
        });
      }
    ...
    const [filasActualizadas] = await OrdenTrabajo.update(datosAActualizar, {
      where: {
        id_orden: id,
    ...
    if (filasActualizadas === 0) {
      return res.status(403).json({
        error:
          "Acceso denegado. La orden no existe o no está asignada a tu usuario.",
      });
    }

    res.json({
      mensaje: "Estado de orden actualizado exitosamente y stock descontado.",
    });
  } catch (error) {
    console.error("Error al actualizar estado:", error);
    res
      .status(500)
      .json({ error: "Fallo interno al actualizar el estado de la orden." });
  }
};
