...
export const getUsuarios = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Error al obtener la lista de usuarios/técnicos." });
  }
};
...
export const createUsuario = async (req, res) => {
  try {
    ...
    const { nombre, apellido, dni, email, telefono, id_rol, password } =
      req.body;
    ...
    if (!nombre || !apellido || !dni || !email || !id_rol || !password) {
      return res
        ...
        .json({
          error:
            "Faltan campos obligatorios (nombre, email, DNI, rol, contraseña).",
        });
    }
    ...
    res
      .status(201)
      .json({ mensaje: "Usuario registrado", usuario: usuarioSinPassword });
  } catch (error) {
...
export const updateUsuario = async (req, res) => {
  try {
    ...
    const { nombre, apellido, dni, email, telefono, id_rol, estado, password } =
      req.body;
