...
export class TecnicoComponent implements OnInit {
  ...
  cargarTecnicos() {
    ...
    this.tecnicoService.getTecnicos().subscribe({
      ...
      error: (err: any) => {
        ...
        this.errorMessage =
          'Error al cargar los datos. Verifique la conexión al backend y la autenticación.';
      },
    });
  }
  ...
  guardarTecnico() {
    ...
    if (
      this.tecnicoSeleccionado.id_usuario === 0 &&
      (!this.tecnicoSeleccionado.password || this.tecnicoSeleccionado.password.length === 0)
    ) {
    ...
    if (this.tecnicoSeleccionado.id_usuario === 0) {
      ...
      this.tecnicoService.createTecnico(dataToCreate).subscribe({
        ...
        error: (err) => {
          this.errorMessage =
            ...
            (err.error?.error || 'Verifique los datos (DNI/Email duplicado).');
      ...
      this.tecnicoService.updateTecnico(dataToUpdate).subscribe({
        ...
        error: (err) => {
          this.errorMessage =
            'Error al actualizar el técnico: ' + (err.error?.error || 'Verifique los datos.');
  ...
  eliminarTecnico(id_usuario: number) {
    if (confirm('¿Estás seguro de que quieres eliminar este técnico?')) {
      ...
      this.tecnicoService.deleteTecnico(id_usuario).subscribe({
        ...
        error: (err) => {
          this.errorMessage =
            'Error al eliminar el técnico: ' + (err.error?.message || err.message);
        },
      });
    }
  }
